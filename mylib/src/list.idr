...
||| Two lists are equal if their elements correspond and they are equal.
...
||| A function that returns a nat the length of a given list
export
...
export
list_filter_even': List Nat -> List Nat
list_filter_even' l = list_filter nat_even l

export
list_filter_True': List Bool -> List Bool
list_filter_True' l = list_filter bool_id l

||| Function that reduces a list of values into a list of ones
export
const_one: List a -> List Nat
const_one Nil = Nil
const_one (Cons h t) =
  (Cons nat_one (const_one t))

||| Function that returns the length of a given list
||| but it works by first reducing a list of values to a list of ones
||| and then it uses foldr to add up those numbers
export
list_length': List Nat -> Nat
list_length' a = list_fold_right nat_plus nat_zero (const_one a)

||| Returns True if all of natural numbers in a given list are even
||| and False otherwise
export
allEven: List Nat -> Bool
allEven a = list_fold_right bool_and True (list_map nat_even a)

||| Returns True if any of the natural numbers in a given list are even
||| and False otherwise.
export
someEven: List Nat -> Bool
someEven a = list_fold_right bool_and False (list_map nat_even a)

||| Returns the number of natural numbers in a given list that are even
export
numEven: List Nat -> Nat
numEven a = list_length' (list_filter nat_even a)
